...
<xsl:stylesheet version="4.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                ...
                xmlns:measures="http://www.thalesgroup.com/sonar/measures/v3"
                ...
                xmlns:size="http://www.thalesgroup.com/sonar/size/v2"
                xmlns:memory="http://www.thalesgroup.com/sonar/memory/v1"
                xmlns:sonar="http://www.thalesgroup.com/sonar/v5" xmlns:xs="http://www.w3.org/2001/XMLSchema">
    ...
    <xsl:template match="*">
        ...
        <xsl:if test="$eltName='tusar'">
            <sonar:sonar xmlns:xs="http://www.w3.org/2001/XMLSchema"
                         ...
                         xmlns:size="http://www.thalesgroup.com/sonar/size/v2"
                         xmlns:memory="http://www.thalesgroup.com/sonar/memory/v1"
                         xmlns:sonar="http://www.thalesgroup.com/sonar/v5"
                         version="5.0">
                <xsl:apply-templates select="node()"/>
            </sonar:sonar>
        </xsl:if>
        ...
        <xsl:if test="$namespace='measures'">
            ...
            <xsl:if test="local-name()='memory'">
                <xsl:element name="measures:{local-name()}">
                    ...
                    <xsl:apply-templates select="node()">
                        <xsl:with-param name="namespace">memory</xsl:with-param>
                    </xsl:apply-templates>
                </xsl:element>
            </xsl:if>
        ...
        <xsl:if test="$namespace='memory'">
            <xsl:element name="memory:{local-name()}">
                <xsl:for-each select="@*">
                    <xsl:attribute name="{name()}">
                        <xsl:value-of select="."/>
                    </xsl:attribute>
